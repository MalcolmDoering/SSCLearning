...
class SimpleFeedforwardNetworkSplitOutputs2(object):
    # same as SimpleFeedforwardNetworkSplitOutputs but an additional output telling which participant should act
    #     
    def __init__(self, 
                 inputDim, 
                 inputSeqLen, 
        ...
        self.numSpeechOutputClasses = numSpeechOutputClasses
        self.numMotionOutputClasses = numMotionOutputClasses
        self.numParticipantOutputClasses = numParticipantOutputClasses
        self.batchSize = batchSize
        self.embeddingSize = embeddingSize
        self.seed = seed
        self.outputSpeechClassWeights = outputSpeechClassWeights
        ...
        self.shopkeeper_participant_action_id_targets = tf.placeholder(tf.int32, [self.batchSize, ], name="participant_action_id_targets")
        self.shopkeeper_participant_action_id_targets_onehot = tf.one_hot(self.shopkeeper_participant_action_id_targets, self.numParticipantOutputClasses)
        

        # class weights
        self.outputSpeechClassWeightsTensor = tf.placeholder(tf.float32, [self.numSpeechOutputClasses], name="speech_action_weights")
        self.outputMotionClassWeightsTensor = tf.placeholder(tf.float32, [self.numMotionOutputClasses], name="motion_action_weights")
        self.outputParticipantClassWeightsTensor = tf.placeholder(tf.float32, [self.numParticipantOutputClasses], name="participant_action_weights")
        ...
        with tf.variable_scope("speech_and_spatial_output/shopkeeper_action_decoder"):
            ...
            #
            self.output_participant_action_decoder = tf.layers.dense(inputs_condensed_reshaped_condensed, 
                                                             self.numParticipantOutputClasses, 
                                                             activation=tf.nn.leaky_relu, use_bias=True, kernel_initializer=tf.initializers.he_normal())    
            self.output_participant_action_softmax = tf.nn.softmax(self.output_participant_action_decoder)
            self.output_participant_action_argmax = tf.argmax(self.output_participant_action_softmax, axis=1)
        ...
        with tf.variable_scope("loss"):
            ...
            #
            participantActionLossWeights = tf.squeeze(tf.gather_nd(tf.expand_dims(self.outputParticipantClassWeightsTensor, 1), tf.expand_dims(self.shopkeeper_participant_action_id_targets, 1)))
            participantActionLossWeights = tf.dtypes.cast(self.output_mask, tf.float32) * participantActionLossWeights
            
            self.output_participant_action_loss = tf.losses.softmax_cross_entropy(self.shopkeeper_participant_action_id_targets_onehot, 
                                                                          self.output_participant_action_decoder,
                                                                          weights=participantActionLossWeights,
                                                                          reduction=tf.losses.Reduction.SUM_OVER_BATCH_SIZE)

            self.loss = self.output_speech_action_loss + self.output_motion_action_loss + self.output_participant_action_loss
    ...
    def train(self, 
        ...
        all_shopkeeper_speech_action_loss = [] 
        all_shopkeeper_motion_action_loss = []
        all_shopkeeper_participant_action_loss = [] 
        ...
        for sei in batchStartEndIndices:
            ...
            feedDict = {self.input_sequences: inputSequenceVectors[sei[0]:sei[1]], 
                        self.shopkeeper_speech_action_id_targets: outputSpeechActionIds[sei[0]:sei[1]], 
                        self.shopkeeper_motion_action_id_targets: outputMotionActionIds[sei[0]:sei[1]], 
                        self.shopkeeper_participant_action_id_targets: outputParticipantActionIds[sei[0]:sei[1]],
                        self.output_mask: outputMasks[sei[0]:sei[1]],
                        self.outputSpeechClassWeightsTensor: self.outputSpeechClassWeights,
                        self.outputMotionClassWeightsTensor: self.outputMotionClassWeights
                        }
            
            loss, shopkeeper_speech_action_loss, shopkeeper_motion_action_loss, shopkeeper_participant_action_loss, _ = self.sess.run([self.loss, self.output_speech_action_loss, self.output_motion_action_loss, self.output_participant_action_loss, self.train_op], feed_dict=feedDict)
            ...
            all_shopkeeper_speech_action_loss.append(shopkeeper_speech_action_loss) 
            all_shopkeeper_motion_action_loss.append(shopkeeper_motion_action_loss)
            all_shopkeeper_participant_action_loss.append(shopkeeper_participant_action_loss)

        return all_loss, all_shopkeeper_speech_action_loss, all_shopkeeper_motion_action_loss, all_shopkeeper_participant_action_loss
    ...
    def get_loss(self, 
        ...
        for sei in batchStartEndIndices:
            ...
            loss, shopkeeper_speech_action_loss, shopkeeper_motion_action_loss, shopkeeper_participant_action_loss = self.sess.run([self.loss, self.output_speech_action_loss, self.output_motion_action_loss, self.output_participant_action_loss], feed_dict=feedDict)
    ...
    def predict(self, 
        ...
        for sei in batchStartEndIndices:
            ...
            predShkpSpeech, predShkpMotion, predShkpParticipant = self.sess.run([self.output_speech_action_argmax, self.output_motion_action_argmax, self.output_participant_action_argmax], feedDict)
            
            ###
            
            all_predShkpSpeech.append(predShkpSpeech)
            all_predShkpMotion.append(predShkpMotion)
            all_predShkpParticipant.append(predShkpParticipant)
        

        all_predShkpSpeech = np.concatenate(all_predShkpSpeech)
        all_predShkpMotion = np.concatenate(all_predShkpMotion)
        all_predShkpParticipant = np.concatenate(all_predShkpParticipant)

        return all_predShkpSpeech, all_predShkpMotion, all_predShkpParticipant
